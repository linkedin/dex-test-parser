...
class EncodedValue(byteBuffer: ByteBuffer) {
    ...
    init {
        ...
        when (valueType) {
            ...
            VALUE_SHORT -> value = ByteArray(sizeOf(valueArg), { index -> byteBuffer.get() })
            VALUE_CHAR -> value = ByteArray(sizeOf(valueArg), { index -> byteBuffer.get() })
            VALUE_INT -> value = ByteArray(sizeOf(valueArg), { index -> byteBuffer.get() })
            VALUE_LONG -> value = ByteArray(sizeOf(valueArg), { index -> byteBuffer.get() })
            VALUE_FLOAT -> value = ByteArray(sizeOf(valueArg), { index -> byteBuffer.get() })
            VALUE_DOUBLE -> value = ByteArray(sizeOf(valueArg), { index -> byteBuffer.get() })
            VALUE_STRING -> value = ByteArray(sizeOf(valueArg), { index -> byteBuffer.get() })
            VALUE_TYPE -> value = ByteArray(sizeOf(valueArg), { index -> byteBuffer.get() })
            VALUE_FIELD -> value = ByteArray(sizeOf(valueArg), { index -> byteBuffer.get() })
            VALUE_METHOD -> value = ByteArray(sizeOf(valueArg), { index -> byteBuffer.get() })
            VALUE_ENUM -> value = ByteArray(sizeOf(valueArg), { index -> byteBuffer.get() })
